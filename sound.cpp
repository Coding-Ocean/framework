...
int loadSound( const char* fileName, int usePath ){
    ...
    //  データパス＋ファイル名 編集(mmioOpenがconst char* を受け取ってくれないのでstringを使わない)
    int l = 0;
    if( usePath ){
        l = strlen( DATA_SOUNDS_PATH );
    }
    l += strlen( fileName ) + 1;
...
int loadSoundFromMemory( const char* fileName, int useDefaultPath ){
    ...
    char* pathFileName = new char[ len ];
    pathFileName[ 0 ] = 0;
    if( useDefaultPath ) strcpy_s( pathFileName, len, DATA_SOUNDS_PATH );
    strcat_s( pathFileName, len, fileName );
    //ファイル読み込み
    FILE *fp;
    fopen_s( &fp, pathFileName, "rb" );
    WARNING( fp == 0, "Wavファイルを読み込めません", pathFileName );
    fseek( fp, 0L, SEEK_END );//ファイルポインタをファイルの最後まで進める
    int waveDataSize = ftell( fp );//ファイルサイズを取得
    rewind( fp );//ファイルポインタを最初に戻す
    //バッファを確保して一気に読み込む
    char* waveData = new char[ waveDataSize ];
    fread( waveData, 1, waveDataSize, fp );
    fclose( fp );
    ...
    //fmtChunk-----------------------------------
    while( (unsigned)pWaveData - (unsigned)waveData < riffChunkSize+8 ){
        if( *(pWaveData+0) == 'f' &&
            *(pWaveData+1) == 'm' &&
            *(pWaveData+2) == 't' &&
            *(pWaveData+3) == ' ' ){
    ...
    WARNING( (unsigned)pWaveData - (unsigned)waveData >= riffChunkSize+8, "fmt chunkがない", "" );
    memcpy( fourChar, pWaveData, 1*4 ); pWaveData += 4;
    ...
    WARNING( (unsigned)pWaveData - (unsigned)waveData >= riffChunkSize+8, "data chunkがない", "" );
    memcpy( fourChar, pWaveData, 1*4 ); pWaveData += 4;
    WARNING( strcmp( fourChar, "data" ) != 0, "data chunkでない", "" );
    unsigned long dataChunkSize = 0;
    memcpy( &dataChunkSize, pWaveData, 4*1 ); pWaveData += 4;
    unsigned hoge = ( pWaveData - waveData - 8 + dataChunkSize );//debug
    unsigned piyo = ( hoge == riffChunkSize );//debug
